...
{
    public GameObject itemPrefab; // bone의 프리팹
    // 랜덤 좌표 설정
    ...
    void Start(){
        if (itemPrefab == null)
        {
            Debug.LogError("⚠️ itemPrefab이 설정되지 않았습니다! " +
                           "Inspector에서 itemPrefab 슬롯에 BonePrefab을 할당하세요.");
            return; 
        }

        Debug.Log("✅ RandomPrefabSpawner Start 실행됨");
    ...
    void SpawnRandomSprite()
    {
        ...
        Debug.Log($"📌 뼈다구 생성! 좌표: {randomX}, {randomY}");
